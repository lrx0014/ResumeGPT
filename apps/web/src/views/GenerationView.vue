  ...
  <div class="page generation-detail">
    ...
    <template v-else-if="run">
      ...
      <div v-if="showEdit" class="modal-backdrop" @click.self="showEdit = false"><section class="panel generation-modal" role="dialog" aria-modal="true" aria-labelledby="edit-generation-title"><header class="modal-header"><div><p class="eyebrow">{{ t('generate.detail.editModal.eyebrow') }}</p><h2 id="edit-generation-title">{{ t('generate.detail.editRegenerate') }}</h2></div><button class="modal-close" type="button" :aria-label="t('common.close')" :disabled="submitting" @click="showEdit = false">×</button></header><p class="edit-help">{{ t('generate.detail.editModal.helpText') }}</p><form @submit.prevent="regenerate"><div class="form-grid">
        ...
        <label><span>{{ t('generate.modal.profileLabel') }}</span><select v-model="editForm.profileId" required><option value="" disabled>{{ t('generate.modal.profilePlaceholder') }}</option><option v-for="item in profiles" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>{{ t('generate.modal.opportunityLabel') }}</span><input :value="title" disabled /></label>
        <label><span>{{ t('generate.modal.templateLabel') }}</span><select v-model="editForm.templateId"><option value="">{{ t('generate.modal.templateNone') }}</option><option v-for="item in matchingTemplates" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>{{ t('generate.modal.pageTargetLabel') }}</span><select v-model="editForm.pageTarget"><option value="one_page">{{ t('generate.pageTarget.onePage') }}</option><option value="two_pages">{{ t('generate.pageTarget.twoPages') }}</option><option value="flexible">{{ t('generate.pageTarget.flexible') }}</option></select></label>
