...
const title = computed(() => { const item = opportunities.value.find(value => value.id === run.value?.opportunityId); return item ? [item.title, item.company].filter(Boolean).join(' · ') : 'Generation details' })
const profileName = computed(() => profiles.value.find(value => value.id === run.value?.profileId)?.name ?? 'Deleted profile')
const templateName = computed(() => run.value?.templateId ? templates.value.find(value => value.id === run.value?.templateId)?.name ?? 'Deleted template' : 'Designed without a template')
const stageLabel = computed(() => run.value ? run.value.state === 'failed' ? 'Needs attention' : ({ queued: 'Waiting to start', writing: 'Writing content', rendering: run.value.templateId ? 'Applying template' : 'Designing document', reviewing: 'Reviewing the PDF', repairing: `Repairing layout (${run.value.repairCount}/2)`, finalizing: 'Finalizing PDF', ready: 'Ready' } as Record<string, string>)[run.value.stage] ?? run.value.stage : '')
...
const stepTitle = (step: GenerationStep) => ({ writer_draft: 'Writer draft', rendered_pdf: step.repairCount ? `Rendered PDF · repair ${step.repairCount}` : run.value?.templateId ? 'Template-applied PDF' : 'Designer-rendered PDF', reviewer_feedback: step.repairCount ? `Reviewer feedback · round ${step.repairCount + 1}` : 'Reviewer feedback', user_prompt: 'Your follow-up instruction', system_warning: 'Workflow notice', configuration_change: 'Configuration updated' } as Record<string, string>)[step.kind]
...
async function loadAll() {
  ...
  } catch (cause) { error.value = cause instanceof Error ? cause.message : 'Could not load this generation.' }
...
async function regenerate() {
  ...
  } catch (cause) { error.value = cause instanceof Error ? cause.message : 'Could not update and regenerate this document.' }
...
<template>
  <div class="page generation-detail">
    <PageHeader :title="title" description="Follow the complete writing, document design, review, and revision timeline."><RouterLink class="button" to="/generate">Back to CVs and cover letters</RouterLink></PageHeader>
    <p v-if="error" class="notice error">{{ error }}</p>
    <div v-if="loading" class="empty-state">Loading generation…</div>
    <template v-else-if="run">
      <section class="panel run-summary"><div><span class="status-pill" :class="run.state">{{ stageLabel }}</span><h2>{{ run.documentType === 'resume' ? 'Résumé' : 'Cover letter' }}</h2><p>{{ profileName }} · {{ templateName }} · {{ run.pageTarget.replace('_', ' ') }} · {{ run.language }}</p></div><div class="summary-actions"><button v-if="!active" class="button" type="button" :disabled="submitting" @click="openEditor">Edit &amp; regenerate</button><button v-if="run.state === 'failed'" class="button" :disabled="submitting" @click="retry">Retry</button><button v-if="run.state === 'ready'" class="button primary" @click="download">Download PDF</button></div></section>
      ...
      <section class="detail-grid">
        ...
        <aside class="panel preview-column"><div class="section-title"><div><p class="eyebrow">Artifact</p><h2>{{ selectedStep ? 'Stage PDF' : 'Final PDF' }}</h2></div><button v-if="selectedStep && run.state === 'ready'" class="text-button" @click="selectedStep = undefined">Show final</button></div><GenerationPdfPreview v-if="run.state === 'ready' || selectedStep" :generation-id="run.id" :step-id="selectedStep"/><div v-else class="empty-state compact">A PDF preview appears after the document layout is rendered.</div></aside>
      </section>

      <section v-if="run.state === 'ready'" class="panel revision-panel"><div><p class="eyebrow">Refine</p><h2>Ask for another change</h2><p>Describe what should change. The model receives the grounded draft and current document source, then adds the new result to this timeline.</p></div><form @submit.prevent="revise"><textarea v-model="prompt" rows="4" maxlength="4000" placeholder="For example: Reduce whitespace above Experience and emphasize the Go migration work." required/><div><small>{{ prompt.length }}/4000</small><button class="button primary" :disabled="submitting || !prompt.trim()">{{ submitting ? 'Queuing…' : 'Send revision' }}</button></div></form></section>

      <div v-if="showEdit" class="modal-backdrop" @click.self="showEdit = false"><section class="panel generation-modal" role="dialog" aria-modal="true" aria-labelledby="edit-generation-title"><header class="modal-header"><div><p class="eyebrow">Generation configuration</p><h2 id="edit-generation-title">Edit &amp; regenerate</h2></div><button class="modal-close" type="button" aria-label="Close" :disabled="submitting" @click="showEdit = false">×</button></header><p class="edit-help">Changing these inputs starts a fresh generation. Existing timeline records and stage PDFs are preserved.</p><form @submit.prevent="regenerate"><div class="form-grid">
        <label><span>Document type</span><select v-model="editForm.documentType"><option value="resume">Résumé</option><option value="cover_letter">Cover letter</option></select></label><label><span>Output language</span><input v-model="editForm.language" required maxlength="40" /></label>
        <label><span>Profile</span><select v-model="editForm.profileId" required><option value="" disabled>Select a profile</option><option v-for="item in profiles" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>Job opportunity</span><input :value="title" disabled /></label>
        <label><span>Template</span><select v-model="editForm.templateId"><option value="">No template — let AI design it</option><option v-for="item in matchingTemplates" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>Page target</span><select v-model="editForm.pageTarget"><option value="one_page">One page</option><option value="two_pages">Two pages</option><option value="flexible">Flexible</option></select></label>
        ...
        <fieldset class="full model-card"><legend>{{ editForm.pipelineMode === 'single' ? 'Workflow model' : 'Writer model' }}</legend><div class="model-row"><label><span>Provider</span><select v-model="editForm.writer.connectionId" @change="changeConnection(editForm.writer)"><option value="" disabled>Select provider</option><option v-for="item in connections" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>Model</span><select v-if="models[editForm.writer.connectionId]?.length" v-model="editForm.writer.model" required><option value="" disabled>Select model</option><option v-if="editForm.writer.model && !models[editForm.writer.connectionId].includes(editForm.writer.model)" :value="editForm.writer.model">{{ editForm.writer.model }}</option><option v-for="model in models[editForm.writer.connectionId]" :key="model" :value="model">{{ model }}</option></select><input v-else v-model="editForm.writer.model" required :disabled="loadingModelConnections[editForm.writer.connectionId]" :placeholder="loadingModelConnections[editForm.writer.connectionId] ? 'Loading models…' : 'Enter model name'" /></label></div></fieldset>
        <template v-if="editForm.pipelineMode === 'multi'"><fieldset class="full model-card"><legend>{{ editForm.templateId ? 'Template applier' : 'Document designer' }}</legend><div class="model-row"><label><span>Provider</span><select v-model="editForm.renderer.connectionId" @change="changeConnection(editForm.renderer)"><option value="" disabled>Select provider</option><option v-for="item in connections" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>Model</span><select v-if="models[editForm.renderer.connectionId]?.length" v-model="editForm.renderer.model" required><option value="" disabled>Select model</option><option v-if="editForm.renderer.model && !models[editForm.renderer.connectionId].includes(editForm.renderer.model)" :value="editForm.renderer.model">{{ editForm.renderer.model }}</option><option v-for="model in models[editForm.renderer.connectionId]" :key="model" :value="model">{{ model }}</option></select><input v-else v-model="editForm.renderer.model" required :disabled="loadingModelConnections[editForm.renderer.connectionId]" :placeholder="loadingModelConnections[editForm.renderer.connectionId] ? 'Loading models…' : 'Enter model name'" /></label></div></fieldset><fieldset class="full model-card"><legend>Visual reviewer</legend><div class="model-row"><label><span>Provider</span><select v-model="editForm.reviewer.connectionId" @change="changeConnection(editForm.reviewer)"><option value="" disabled>Select provider</option><option v-for="item in connections" :key="item.id" :value="item.id">{{ item.name }}</option></select></label><label><span>Model</span><select v-if="models[editForm.reviewer.connectionId]?.length" v-model="editForm.reviewer.model" required><option value="" disabled>Select model</option><option v-if="editForm.reviewer.model && !models[editForm.reviewer.connectionId].includes(editForm.reviewer.model)" :value="editForm.reviewer.model">{{ editForm.reviewer.model }}</option><option v-for="model in models[editForm.reviewer.connectionId]" :key="model" :value="model">{{ model }}</option></select><input v-else v-model="editForm.reviewer.model" required :disabled="loadingModelConnections[editForm.reviewer.connectionId]" :placeholder="loadingModelConnections[editForm.reviewer.connectionId] ? 'Loading models…' : 'Enter model name'" /></label></div></fieldset></template>
