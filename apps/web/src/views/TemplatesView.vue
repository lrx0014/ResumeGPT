...
async function upload() {
  ...
  try {
    ...
    notice.value = 'Template uploaded. Text extraction, security scanning, and PDF preview generation are running.'
...
async function remove(item: Template) {
  if (!window.confirm(`Delete the “${item.name}” template?`)) return
  try { await api.deleteTemplate(item.id); items.value = items.value.filter(candidate => candidate.id !== item.id); notice.value = 'Template deleted.' }
  ...
  <div class="page templates-page">
    ...
    <div v-else class="card-grid">
      <article v-for="item in items" :key="item.id" class="entity-card template-card">
        ...
        <footer><RouterLink class="text-button" :to="`/templates/${item.id}`">View</RouterLink><div><button v-if="item.state === 'ready'" class="text-button" type="button" @click="download(item)">Download</button><button v-if="!item.builtIn" class="text-button danger-text" type="button" @click="remove(item)">Delete</button></div></footer>
      </article>
    </div>
  </div>
</template>
