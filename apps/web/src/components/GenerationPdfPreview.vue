...
async function load(){loading.value=true;error.value='';release();try{previewUrl.value=URL.createObjectURL(await api.generationPreview(props.generationId,props.stepId))}catch(cause){error.value=cause instanceof Error?cause.message:t('generate.pdfPreview.error')}finally{loading.value=false}}
