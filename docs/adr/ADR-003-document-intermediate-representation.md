...
Allowing an LLM to directly generate DOCX or TeX mixes profile-derived content, document structure, template syntax, and layout. That makes validation, multi-format output, version comparison, and template-injection prevention unreliable.
...
- The selected `profile_id`, supporting profile excerpts, and grounding state for each material claim.
...
Generation inputs reference immutable snapshots. Revisions use domain commands or an allowlisted subset of JSON Patch paths. They cannot modify tenant identifiers, supporting profile references, or system validation fields.
...
- **HTML as the only IR:** Expressive, but weak in document semantics and supporting-profile references, and unstable for DOCX conversion.
