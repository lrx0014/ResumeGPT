...
\usepackage{iftex} % detects which engine is compiling this document
%-----------------------------------------
...
%----------FONT OPTIONS-------------------
% Prefer Source Sans Pro when installed, with a portable sans-serif fallback.
...
% CJK support: under XeTeX, render Chinese/Japanese/Korean text with a Noto
% CJK font while leaving the Latin body font untouched, so bilingual and
% fully CJK resumes render correctly instead of showing missing-glyph boxes.
\ifXeTeX
  \usepackage{fontspec}
  \usepackage{xeCJK}
